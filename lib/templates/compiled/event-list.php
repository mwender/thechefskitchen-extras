...
use \LightnCandy\SafeString as SafeString;use \LightnCandy\Runtime as LR;return function ($in = null, $options = null) {
    ...
    $inary=is_array($in);
    return ''.LR::sec($cx, (($inary && isset($in['events'])) ? $in['events'] : null), null, $in, true, function($cx, $in) {$inary=is_array($in);return '<div class="row event desktop">
  <div class="col-sm-2">
...
';}).'  </ul>
   <p><strong>Location:</strong><br/><a href="'.htmlspecialchars((string)((isset($in['location']) && is_array($in['location']) && isset($in['location']['link'])) ? $in['location']['link'] : null), ENT_QUOTES, 'UTF-8').'" target="_blank">'.htmlspecialchars((string)((isset($in['location']) && is_array($in['location']) && isset($in['location']['name'])) ? $in['location']['name'] : null), ENT_QUOTES, 'UTF-8').'<br>'.((isset($in['location']) && is_array($in['location']) && isset($in['location']['address'])) ? $in['location']['address'] : null).'</a></p>
  </div>
</div>
